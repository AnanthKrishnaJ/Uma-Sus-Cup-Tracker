...
suscup6_race = {
    ...
    "date": "2025-09-03",
    "images": ["images/races/6.1.png", "images/races/6.2.png", "images/races/6.3.png", "images/races/6.4.png"],
    "time": "18:18",
    "roomId": "9075 4387",
...
# 1. Add winner
# Find INITIAL_DATA = { ... winners: [
winners_start = content.find('winners: [')
if winners_start != -1:
    end_of_winners = content.find('],', winners_start)
    winners_list = content[winners_start + 10:end_of_winners]
    # Check if Sus Cup 6 is already there
    if 'Sus Cup 6' not in winners_list:
        new_winner_str = '\n                ' + json.dumps(suscup6_winner) + ','
        # Add to beginning
        content = content[:winners_start+10] + new_winner_str + content[winners_start+10:]

# 2. Add race to INITIAL_DATA.races
races_idx = content.find('races: [')
if races_idx != -1:
    # Append after races: [
    race_json = json.dumps(suscup6_race, indent=20)
    # Fix the indentation format of the generated JSON string
    formatted_race = ""
    ...
    if 'SUS CUP 6' not in content[races_idx:content.find('],', races_idx)]:
        content = content[:races_idx+8] + formatted_race + content[races_idx+8:]

# 3. Add NPCs to NPC_PROFILES
npc_profiles_start = content.find('const NPC_PROFILES = {')
npc_profiles_end = content.find('};', npc_profiles_start)
npc_block = content[npc_profiles_start:npc_profiles_end]

for npc in ["Ogress", "Ribbon Nocturne", "Maleficus"]:
    if f'"{npc}":' not in npc_block:
        npc_str = f'\n            ,"{npc}": {{ name: "{npc}", type: "NPC Uma", characterUrl: null, imageUrl: null }}'
        # inject before the end
        content = content[:npc_profiles_end] + npc_str + content[npc_profiles_end:]

# 4. Modify App.resetData to wipe localStorage so it reloads INITIAL_DATA
# This is a one-off run for the script to trigger a reload of localstorage for the user
import re
content = re.sub(
    r"this\.loadData\(\);\s*this\.setupNavigation\(\);",
    r"localStorage.removeItem('sus_cup_db'); this.loadData(); this.setupNavigation();",
    content
)
...
print("Added Sus Cup 6 data successfully.")
