...
"special-week": { id: "special-week", name: "Special Week", type: "Uma Musume", version: "Original / Default", url: "https://gametora.com/umamusume/characters/special-week", image: "https://gametora.com/images/umamusume/characters/chara_stand_1001_100101.png" },
            ...
            "oguri-cap-christmas": { name: "Oguri Cap (Christmas)", url: "https://gametora.com/umamusume/characters/100602-oguri-cap", image: "https://gametora.com/images/umamusume/characters/chara_stand_1006_100602.png" },
        ...
        const App = {
            ...
            buildLeaderboards(racesArray) {
                ...
                racesArray.forEach(race => {
                    race.participants.forEach(p => {
                        if (!players[p.player]) {
                            players[p.player] = {
                                name: p.player, runs: 0, w: 0, secondPlaces: 0, thirdPlaces: 0, fourthPlaces: 0, fifthPlaces: 0,
            ...
            renderDashboard() {
                ...
                const winnerRows = championshipWinners.map((winner, index) => {
                    ...
                    <div class="${rowClass}">
                        ...
                        <div class="champ-winner"><i class="fa-solid fa-trophy" style="color:#f1c40f; margin-right:6px;"></i> ${winner.playerStr}</div>
                    </div>
                    `;
                }).join('');
            ...
            renderArchive() {
                ...
                races = races.filter(race => {
                    if (searchTxt && !race.name.toLowerCase().includes(searchTxt) && !String(race.cupNumber).toLowerCase().includes(searchTxt)) return false;
    ...
    document.getElementById('race-detail-content').innerHTML = html;
        ...
        const generateAvatarHTML = (p, bestUma) => {
            if (bestUma && bestUma.umaId) {
                if (UMA_DATABASE[bestUma.umaId] && UMA_DATABASE[bestUma.umaId].image) {
                    return `<img src="${UMA_DATABASE[bestUma.umaId].image}" onerror="this.style.display='none'; this.parentElement.innerText='${p.name.substring(0,2).toUpperCase()}';" alt="${bestUma.name}">`;
                }
                let imgUmaId = bestUma.umaId;
                    ...
                    html += `
                        <div class="list-item">
                            ...
                            <div class="item-content">
                                ...
                                <div class="item-subtitle">${h.race} (Sus Cup ${h.cup}) &bull; ${h.strategy} &bull; Time: ${h.time || 'N/A'}</div>
                            </div>
                        </div>
                    `;
                });
