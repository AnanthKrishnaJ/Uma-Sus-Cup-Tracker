...
"special-week": { id: "special-week", name: "Special Week", type: "Uma Musume", version: "Original / Default", url: "https://gametora.com/umamusume/characters/special-week", image: "https://gametora.com/images/umamusume/characters/chara_stand_1001_100101.png" },
        ...
        const App = {
            ...
            renderDashboard() {
                ...
                const latestWinner = latestRace?.participants.find(p => (p.position || p.pos) === 1 || (p.position || p.pos) === '1');
                        ...
                        <table class="table" style="font-size: 0.85rem; width: 100%;">
                            ...
                            <tbody>
                                ${sortedParticipants.map(p => `
                                    <tr>
                                        <td><strong>${p.position || p.pos || '-'}</strong></td>
                                        <td>
                                            <div style="display:flex; align-items:center; gap:8px;">
                                                ${App.getUmaImage(p.umaId || p.uma).replace('width:40px;height:40px', 'width:30px;height:30px').replace('border-radius:4px', 'border-radius:50%')}
                                                <span>${p.uma}</span>
                                            </div>
                                        </td>
                                        <td><a href="#" onclick="App.showPlayer('${p.player}'); return false;" style="color:var(--primary); font-weight:bold; text-decoration:none;">${p.player}</a></td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
